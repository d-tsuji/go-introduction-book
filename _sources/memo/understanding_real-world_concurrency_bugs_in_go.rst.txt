...
.. image:: Table_9_Root_causes_of_non-blocking_bugs.PNG

6.1.1 Failing to Protect Shared Memory
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

* 半数以上は従来(CやJava)同様、原子性違反や順序違反、データ競合によるもの
* ``go`` でゴルーチンを簡単に作れるが、データ共有のバグを増やす可能性がある

.. code-block:: diff
    :caption: Docker Figure 8. A data race caused by anonymous function.
...
.. code-block:: diff
    :caption: CockroachDB Figure 9. A non-blocking bug caused by misusing WaitGroup.
...
* 開発者は dur が 0 より大きい、または ``ctx.Done()`` が呼ばれた場合のみ関数から return することを意図している
    * Fix前は dur が 0 以下の場合に処理をすり抜けてしまう、というバグ
